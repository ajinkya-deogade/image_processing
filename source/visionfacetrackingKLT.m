...
% bbox=[151.5 281.5 157 150];
% Draw the returned bounding box around the detected face.
% shapeInserter  = vision.ShapeInserter;
% videoFrame = step(shapeInserter, videoFrame, bbox);
% figure; imshow(videoFrame); title('Detected larva');
% K=imcrop(videoFrame,bbox);
% figure; imshow(K); title('Cropped');
...
I  = imread('Ajinkya_colour Photo.jpg');
I2 = rgb2gray(I);
cornerDetector = vision.CornerDetector('Method','Minimum eigenvalue (Shi & Tomasi)');
points = step(cornerDetector,I2);
% % The coordinates of the feature points are with respect to the cropped region. They need to be translated back into the original image
% % coordinate system. 
% points = double(points);
% points(:, 1) = points(:, 1) + double(bboxPolygon(1));
% points(:, 2) = points(:, 2) + double(bboxPolygon(2));
% 
% % % Display the detected points.
markerInserter = vision.MarkerInserter('Shape', 'Plus','BorderColor', 'White');
I2 = step(markerInserter, I2, points);
figure; imshow(I2); title('Detected features'); hold on;
